...
- **One front door — but two planes.** Envoy on 443 is the only port a browser touches, and it is *only* the data plane. The gateway is what assembles the platform. That is why the controller, hub and EDA each have arrows arriving from two different places:
