...
Record the receptor listener address — bookkeeping, not wiring (the actual mesh is the static `receptor.conf` from Lab 12; this row feeds the API and topology view):
...
Two queues, both by percentage — `controlplane` (Lab 7 made it by hostname; same result on one box) and **`default`**, which is where every work node lands and, critically, **the queue AWX picks automatically for job templates with no instance group set**:
